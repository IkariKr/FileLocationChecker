...
- ⚙️ **灵活配置规则**：
  ...
  - `检查文件大小`（默认已勾选）：匹配 A 与 B 中的文件字节大小（Length）。
  - `容差滑块`（默认 `0%`，范围 `0%-100%`）：当勾选“检查文件大小”时激活，允许设定文件字节对比的误差百分比范围（方便忽略尾部换行符、文本字符微调带来的微小尺寸差异）。
  - `排除 A 路径`（默认已勾选）：当 A 为 B 的子文件夹时，在 B 的扫描结果中自动排除 A 路径及其子目录下的文件自身。
  - `md文件检查资源有效性`（默认已勾选）：当检查 `.md` 文件时，自动解析并检查源文件夹 A 与目标文件夹 B 中的 Markdown 文件引用的图片、附件及本地链接资源是否存在。若存在缺失资源，对应列（`SourcePath` 或 `TargetPath`）将以**红色加粗字体**高亮显示，鼠标悬停时弹出 poptip 列出所有缺失的资源列表。
  - `md文件检查外链`（默认已勾选）：当检查 `.md` 文件时，自动解析并检查 A 与 B 中的 Markdown 文件中是否包含 `http/https/ftp` 等网络远程外链。若包含外链，对应列将以**红色加粗字体**高亮显示，鼠标悬停时弹出 poptip 列出所有包含的网络外链清单。
...
- 📊 **可视化数据表格 (DataGrid)**：
  ...
  - **文本文件差异对比 (Diff Viewer)**：选中任意结果行右键点击“比较差异”，弹出代码/文本对比视窗，高亮高能标注新增（+）与删除（-）变动，并计算改动行数。
